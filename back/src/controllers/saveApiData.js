...
const getApiData = async () => { // TODO ESTO ES UN CONTROLADOR , no ruta
    try {
        ...
        while(i < 6){  //voy a recorrer hasta que i sea menor a 6. POR CADA i VOY A HACER UN LLAMADO A LA API Y GUARDANDO UNA PROMESA

            let apiData = await axios(`http://rickandmortyapi.com/api/character?page=${i}`) // me vienen 20 en el link (http://rickandmortyapi.com/api/character) por eso tengo que hacer la logica para hacer 100. TODO ESTO ME DEVUELVE UNA PROMESA
            ...
            let allCharacters = [];
            characters.map(char => { allCharacters = allCharacters.concat // mapeo el array que tengo arriba. dsp piso el array vacio y le concateno char, es decir cada uno de los personajes que tengo arriba 
            (char)})
